...
namespace BiliLite.Services
{
    public class BackgroundDownloadService : IDownloadService
    {
        ...
        private static BackgroundTransferGroup _group = BackgroundTransferGroup.CreateGroup("BiliDownlad");//下载组，方便管理
        ...
        {
            var downloads = await GetCurrentDownloads();
            var download = downloads.FirstOrDefault(x => x.Guid.ToString() == guid);
            if (download != null)
            {
                download.Pause();
            }
        ...
        {
            var downloadOperations = await BackgroundDownloader.GetCurrentDownloadsForTransferGroupAsync(_group);
            return downloadOperations.Select(x => new BackgroundDownloadOperation(x)).ToList();
        }
        ...
        private async Task DownloadVideo(DownloadInfo downloadInfo, DownloadUrlInfo url, StorageFolder episodeFolder)
        {
